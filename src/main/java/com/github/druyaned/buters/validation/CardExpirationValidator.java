...
public class CardExpirationValidator
    ...
    private static final Pattern cardExpirationPattern =
            Pattern.compile("(0[1-9]|1[0-2])/([2-9][0-9])");

    @Override
    public boolean isValid(String cardExpiration, ConstraintValidatorContext cvc) {
        Matcher matcher = cardExpirationPattern.matcher(cardExpiration);
        if (!matcher.matches()) {
            return false;
        }
